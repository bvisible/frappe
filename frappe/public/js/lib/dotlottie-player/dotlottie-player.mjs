//// Neoffice — vendored from unpkg https://unpkg.com/@dotlottie/player-component@2.7.12/dist/dotlottie-player.mjs (version 2.7.12).
//// Upstream frappe loads no Lottie player at all; ours added one so the 404 page and the
//// desk freeze overlay (frappe/public/js/frappe/dom.js) can show a .lottie animation, and
//// pulled it straight from unpkg.com with a <script type="module"> in www/404.html and
//// www/app.html. That put a third-party CDN request on every desk page load and on a
//// public error page, and it broke both offline and behind a restrictive CSP. The bytes
//// now come from our own origin, unchanged.
//// This is the package's code-split ESM build, mirrored whole: the four chunk-*.mjs it
//// imports eagerly, and the nine renderer/feature modules it import()s on demand, all sit
//// beside it so the relative specifiers resolve exactly as they did on unpkg — same files,
//// same lazy loading, same ~124 KB eager instead of the 1.9 MB single-file UMD build.
//// Only two things differ from the published bytes: this header, and the trailing
//// //# sourceMappingURL comment, dropped because the .map files are not vendored and each
//// one would 404. License: MIT (LottieFiles), see the package on npm.
//// Re-vendor with neoffice-devops scripts, or by hand:
////   for f in <the file list in NEOFFICE_FORK_MARKERS.md>; do curl -sSL -o "$f" \
////     "https://unpkg.com/@dotlottie/player-component@2.7.12/dist/$f"; done
//// then re-add these headers and bump the ?v= in www/404.html and www/app.html.
